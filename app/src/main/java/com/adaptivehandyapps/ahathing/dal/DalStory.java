...
//    }
    ...
    public Boolean remove(DaoStory dao, Boolean updateDatabase) {
        ...
        if (mPlayList.getActiveStory().getMoniker().equals(dao.getMoniker())) {
            DaoStory daoReplacement = null;
            // if an object is defined, set as replacement
            if (getDaoRepo().size() > 0) daoReplacement = (DaoStory) getDaoRepo().get(0);
            // set or clear active object
            mPlayList.setActiveStory(daoReplacement);
        }
        // refresh
        if (mCallback != null) mCallback.onRepoProviderRefresh(true);
